...
def estimate_dsm_penalty(
    ...
    # Worst case: all blocks at max deviation
    worst_case = float(
        np.sum(
            vols
            * duration_hours
...
def assess_risk(
    ...
    # Estimate penalties assuming actual demand = bid volumes * (1 + noise)
    np.random.seed(42)
    simulated_demand = [v * (1 + np.random.normal(0, 0.08)) for v in bid_volumes]
